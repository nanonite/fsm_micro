...
DebugPort->write("initializing");
	...
	//if(!sendRecQuickATCommand("AT#STIA=0"))return 0; //sets STN for no unsolicited response
	if(!sendRecQuickATCommand("AT#STIA=2"))return 0; // need for receiving SMS
	DebugPort->write("AT#STIA=0");
	if(!sendRecQuickATCommand("ATV1"))return 0;		//set verbose mode
	DebugPort->write("ATV1");
	if(!sendRecQuickATCommand("AT&K0"))return 0;		//set flow control off
	DebugPort->write("AT&K0");
	//if(!sendRecQuickATCommand("AT+IPR=0"))return 0;		//command is supported for changing baud rate???
	//DebugPort->write("AT+IPR=0");
	if(!sendRecQuickATCommand("AT+CMEE=2"))return 0;	//set extended error report = verbose mode
	DebugPort->write("AT+CMEE=2");
	//if(!sendRecQuickATCommand("AT#SELINT=2"))return 0;	//SELIENT 2 is now standard
	switch(_band){
	...
	default : if(!sendRecQuickATCommand("AT#AUTOBND=1"))return 0; //default autoBand it

	}
	DebugPort->write("AT+AUTOBND=1");
return 1;
}
...
const char* const GSMbase::sendRecQuickATCommand_ATNT(const char* theMessage){
	telitPort.flush();
	telitPort.write(theMessage);
	telitPort.write("\r");
	return	(catchTelitData(5000,true));				// 5000 milli time out with a quickCheck return data
}
...
const char* end){
	...
	size_t subLen=((endP-startP)+1);
	char deadStr[(size_t) (sizeof(char) * subLen)];	// used if you do a double parse	
	uint16_t dataPos=0;
	while ( startP != endP ){			// grab between starP and endP
		deadStr[dataPos++]= *startP++;
	}
	deadStr[dataPos]= '\0';                      	// NULL to make a proper string
	free(parsedData);          			// if it has been allocated, Free the memory
	parsedData=NULL;
	parsedData = (char*) malloc( (size_t)(sizeof(char) * subLen) );      // get memory 
	if (parsedData == NULL) {
	return 0;
	}            					// if we couldn't get mem
	strcpy(parsedData, deadStr);			// don't change this unless you are careful.
