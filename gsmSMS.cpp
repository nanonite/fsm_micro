...
bool gsmSMS::smsInit(){
	...
	//set text mode not PDU
	while(!sendRecQuickATCommand("AT+CMGF=1")){
	}	
	//or
	//set PDU mode not text
	//if(!sendRecQuickATCommand("AT+CMGF=0")) return 0;         
	
	DebugPort->write("AT+CMFG");
	
	//optional, set service center? (only necessary for transmit?)
	/*
	if(!sendRecQuickATCommand("AT+CSCA=+13123149810,145")) return 0;
	DebugPort->write("AT+CSCA");
	*/
	
	//optional, extended text config?
	/* gives text message parameters
	if(!sendRecQuickATCommand("AT+CSMP=17,167,0,240")) return 0;
	DebugPort->write("AT#CSMP");
	*/
		
	//optional, SMSMODE?
	if(!sendRecQuickATCommand("AT#SMSMODE=0")) return 0;				// set extended smsmode off , dumb SIM
	DebugPort->write("AT#SMSMODE");
	
	
	
	//AN+CNMI --- set receive action
	
	//optional configs?
	
	//if(!sendRecQuickATCommand("AT+CNMI=2")) return 0;
	
	//SET to no buffer no notification on receive 
	...
	//SET to buffer  no notification on receive
	//<Result notification sent to DTE, buffer notificaion on TA if DTE busy>,
	//<brodcast notification to TE using PDU format or Text format (+CMT)>,
	//<broadcast Cell Messages to DTE using PDU or Text format(+CBM)>,
	//<when buffering switches state flushes all stored notification to DTE (+CDS) >,
	//<TA buffer of unsolicited result codes>
	//USE AT+CNMI=2,2,2,1,0 to have messages send to DTE on reception.
	
	//deff works
	if(!sendRecQuickATCommand("AT+CNMI=2,2,2,1,0")) return 0;	
	DebugPort->write("AT+CNMI");
...
} //what is sent back is the string between the two parenthesis
//////////////////////////////////////////////////////////////////////LIST MESSAGES FUNCS*
