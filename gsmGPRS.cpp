...
{
	memset(ipAddress1,'\0',IPsize);	memset(ipAddress2,'\0',IPsize);
}
...
//For single use GPRS Context Activation - #GPRS **NEEDS TO BE IMPLEMENTED** 
...
//GET IP
//AT+CGPADDR=<cntx ID>
const char* const gsmGPRS::checkipCGPADDR(const char*const _context){
//RETURNS -> #CGPADDR:0,"10.137.93.60"
	telitPort.write("AT+CGPADDR");			//SEND COMMAND
return sendRecATCommandParse(_context,"\"","\"");	//add context # and parse IP
}
...
//////////////////////////////////////////////////////////////////////////////////////SOCKET CONFIG*
bool gsmGPRS::socketConSCFGEXT(const char* const socketid,const char* const srMode,
const char* const recDataMode,const char* const alive,
const char* const autoRes ,const char* const sendDataMode ){
	telitPort.write("AT#SCFGTEXT=");
	telitPort.write(socketid);
	telitPort.write(",");
	telitPort.write(srMode);
	telitPort.write(",");
	telitPort.write(recDataMode);
	telitPort.write(",");
	telitPort.write(alive);
	telitPort.write(",");
	telitPort.write(autoRes);
	telitPort.write(",");
	telitPort.write(sendDataMode);
	telitPort.write("\r");
return catchTelitData(2000,1);				// look for a OK
}


//FIREWALL SET UP FOR LISTING SOCKET
//FRWL AT#FRWL
//0-> remove 1 ->ADD 2-> drop all
bool gsmGPRS::firewallFRWL(const char* const mode, 
const char* const ipAdress, const char* const mask){
	telitPort.write("AT#FRWL=");
	telitPort.write(mode);
	telitPort.write(",");
	telitPort.write(ipAdress);
	telitPort.write(",");
	telitPort.write(mask);
	telitPort.write("\r");
return catchTelitData(2000,1);				// look for a OK
}
...
//RETURNS: CONNECT
	...
	telitPort.write("\"\r");
	//should this be modified to include <closure type> ?
	/*
	telitPort.write(",");
	telitPort.write(255);
	*/
	DebugPort->write("AT#SD\n");
...
//  shutdown command.
const char* const gsmGPRS::socketStatusSS(const char* const connectionID){
//RETURNS:2,4,91.80.90.162,1000 OK
	telitPort.write("AT#SS");
	telitPort.write(connectionID); //AT#SS=1 is not supported on older firmware
	telitPort.write("\r");
...
bool keepAlive){
	...
	//telitPort.write("GET /UtilTest/getThis.html HTTP/1.0\r\nHOST: www.groundlab.cc\r\n\r\n\r\n");
	telitPort.write("GET ");
	...
	telitPort.write(host);
	telitPort.write("\r\n");
	if(keepAlive)telitPort.write("Connection: keep-alive\r");
	telitPort.write("\r\n\r\n");
	
return  catchTelitData(180000,false,dataSize,4000); 	//Set with two min(180000) general time 
							//out and a 3 sec baud time out.
}
